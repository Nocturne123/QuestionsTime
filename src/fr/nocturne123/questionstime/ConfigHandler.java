...
public class ConfigHandler {
	...
	public static void init(File file) {
		try {
			...
			if(!file.exists()) {
				Asset jarConfigFile = QuestionsTime.getInstance().getContainer().getAsset("defaultconfig.conf").get();
				jarConfigFile.copyToFile(file.toPath());
				QuestionsTime.getInstance().getConsole().sendMessage(
						TextUtils.Console.creatorNormal("The config file wasn't found, the default config file has been created"));
			}
