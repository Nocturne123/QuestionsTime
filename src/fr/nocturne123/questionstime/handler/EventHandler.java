...
public class EventHandler {
	...
	private void handleQuestionAnswer(String message, Player p, QuestionsTime instance, MessageChannelEvent e) {
		...
		if(message.endsWith("qt>"+q.getAnswer())) {
			...
			if(q.isTimed()) instance.stopTimer();
				...
				Task.builder().execute(task -> {
					...
					if(!prize.getItems()[0].getItem().equals(ItemTypes.NONE)) {
						for(int i = 0; i < prize.getItems().length; i++) {
							ItemStack item = prize.getItems()[i];
							p.sendMessage(Text.join(instance.qtPrefix, MessageHandler.get("reward.prize", "quantity:"+item.getQuantity(),
									"modid:"+item.getItem().getId().split(":")[0], "item:"+item.getItem().getId().split(":")[1], "metadata:"+item.toContainer().getValues(true)
			...
			if(q.getMalus().getMoney() > 0 && instance.getEconomy().isPresent()) {
				EconomyService ecoSevice = instance.getEconomy().get();
				p.sendMessage(Text.join(instance.qtPrefix, MessageHandler.get("answer.malus", "money:"+q.getMalus().getMoney(), "currency:"+ecoSevice
						.getDefaultCurrency().getDisplayName().toPlain())));
				Optional<UniqueAccount> account = ecoSevice.getOrCreateAccount(p.getUniqueId());
				if(account.isPresent())
