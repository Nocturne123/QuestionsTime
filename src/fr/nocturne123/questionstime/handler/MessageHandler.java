...
public class MessageHandler {
	...
	public static void init(File file) {
		try {
			if(file != null) {
				...
				if(!file.exists()) {
					Asset jarConfigFile = QuestionsTime.getInstance().getContainer().getAsset("message.conf").get();
					jarConfigFile.copyToFile(file.toPath());
					console.sendMessage(
							TextUtils.Console.creatorNormalWithPrefix("The message file was not found, the default message file has been created"));
					loadDefaultMessages(file);
				} else {
					loadDefaultMessages(file);
					loadMessages(file);
				}
	...
	private static void loadDefaultMessages(File file) {
		...
		messages.put("prize.item", " §9• {quantity} * {modid}§f{item} §b{metadata}");
