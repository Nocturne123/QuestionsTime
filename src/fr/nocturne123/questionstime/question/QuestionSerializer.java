...
public class QuestionSerializer implements TypeSerializer<Question>{
	...
	public void serialize(TypeToken<?> type, Question q, ConfigurationNode node) throws ObjectMappingException {
		...
		if(prize.getTypes().length > 0 && prize.getTypes()[0].getType() != ItemTypes.NONE) {
			...
			for(int i = 0; i < prize.getTypes().length; i++) {
				...
				if(is.getOrCreate(DisplayNameData.class).isPresent())
					isSer += ";"+is.getOrCreate(DisplayNameData.class).get();
				if(is.getOrCreate(LoreData.class).isPresent()) {
					List<Text> lore = is.getOrCreate(LoreData.class).get().asList();
